...
namespace netudt
{
    public class UDTCongestionControl : ICongestionControl {
	...
	protected UDTSession session;
        ...
        private bool slowStartPhase = true;
        ...
        public void SetAckInterval(long ackInterval) {
            ...
            if (session.Socket != null && session.Socket.GetReceiver() != null) {
                session.Socket.GetReceiver().setAckInterval(ackInterval);
            }
        }
        ...
        public void OnACK(long ackSeqno) {
            ...
            if (slowStartPhase) {
                ...
                if (congestionWindowSize > session.FlowWindowSize) {
            ...
            statistics.SendPeriod=packetSendingPeriod;
        }

        private  long PS = UDPEndPoint.DATAGRAM_SIZE;
        private double BetaDivPS = 0.0000015 / UDPEndPoint.DATAGRAM_SIZE;
        ...
        private double ComputeNumOfIncreasingPacket() {
            ...
            if (remaining <= 0) {
                ...
                double exp = Math.Ceiling(Math.Log10(remaining * PS * 8));
        ...
        public void OnLoss(List<int> lossInfo) {
            loss = true;
            long firstBiggestlossSeqNo = lossInfo[0];
            nACKCount++;
            ...
            long currentMaxSequenceNumber = session.Socket.GetSender().GetCurrentSequenceNumber();
            ...
            if (firstBiggestlossSeqNo > lastDecreaseSeqNo) {
                ...
                averageNACKNum = (int)Math.Ceiling(averageNACKNum * 0.875 + nACKCount * 0.125);
                // -reset NAKCount and DecCount to 1, 
                nACKCount = 1;
                decCount = 1;
                /* - compute DecRandom to a random (average distribution) number between 1 and AvgNAKNum */
                decreaseRandom = (int)Math.Ceiling((double)((averageNACKNum - 1) * new Random().Next() + 1));
                // -Update LastDecSeq
